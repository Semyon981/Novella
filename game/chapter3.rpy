...
label startChapter3:
    ...
    menu: #поменял
        "Отложить на 5 минут?"
        "Да":
            "{i}Как буд-то это вообще когда-то было хорошим решением..."
            call prospal from _call_prospal
        "Нет":
            "{i}На первую пару опаздывать слишком опасно. "
    ...
    yasherov "{i}На сегодня закончим на этом, уверен вы и так всë поняли по моим примерам, проблем у ответственного студента возникнуть не должно."
...
label prospal:
    ...
    "{i}Остаётся надеяться только на милость преподавателя в отношении опоздавших."
    ...
    #поменял
    yasherov "{i}Добрый день, можете объяснить, прочему вы опаздываете на первое занятие??"
    menu:
        "{i}Искренне прошу прощения, разобраться в сложной архитектуре этого здания и найти нужную аудиторию оказалось непростой задачей.":
            pass
        "{i}Живот прихватило.":
            pass
        "{i}Да тут не здание а лабиринт, фиг найдëшь, куда идти.":
            pass
        "{i}Надо было перевести бабушку через дорогу.":
            pass
