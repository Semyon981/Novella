...
label startChapter3:
    ...
    yasherov "{cps=45}{i}На сегодня закончим на этом, уверен вы и так всë поняли по моим примерам, проблем у ответственного студента возникнуть не должно."
...
label prospal:
    ...
    #поменял
    yasherov "{cps=45}{i}Добрый день, можете объяснить, прочему вы опаздываете на первое занятие??"
    menu:
        "{cps=45}{i}Искренне прошу прощения, разобраться в сложной архитектуре этого здания и найти нужную аудиторию оказалось непростой задачей.":
            pass
        "{cps=45}{i}Живот прихватило.":
            pass
        "{cps=45}{i}Да тут не здание а лабиринт, фиг найдëшь, куда идти.":
            pass
        "{cps=45}{i}Надо было перевести бабушку через дорогу.":
            pass
