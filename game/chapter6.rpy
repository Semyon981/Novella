...
label startChapter6:
    ...
    play music "music/nornikel.wav" fadein 3.0
    pause 4.0

    "{cps=10}{font=Gilroy-ExtraBold.otf}ГЛАВА VI. ЧИСЛЕННЫЕ МЕТОДЫ В КОМПЬЮТЕРНЫХ ВЫЧИСЛЕНИЯХ"
    
    
    scene auditoria_chislaki with fade

    "{i}Настал тот час, которого боятся все студенты ИСиТа.."
    "{i}Их нога ступила на порог зловещей башни грозного Норникеля Виктора Петровича."
    "{i}Вокруг гнетущая атмосфера и покрытые печалью лица людей, пришедших сдавать лабораторные."
    "{i}Не видя среди них ни одного радостного, вы понимаете, что пока ни у кого лабы так и не приняли."
    ...
    "{i}Толпа измученных и едва живых студентов выползает из аудитории. Во взгляде каждого из них читается: 'Бегите, вы не знаете, куда пришли!!!'"
...
label trueAnswer1:
    norin "Молодец, видно, что не игнорируешь преподавателя, ответишь ещë на один и считай зачëт у тебя в кармане."
    return