...
label startChapter2:
    scene black
    with fade
    "{font=Gilroy-ExtraBold.otf}ГЛАВА II. ТПР"

    scene university
    with fade

    "{cps=45}{i}Следующая пара - теория принятия решений."
    ...
    student "{cps=45}Да я в лучшем случае только половину слов понимаю, откуда терминов столько, я же не физике или дискретке сижу..."
    ...
    if a == 0:
        olga "{cps=45}Это только первое занятие, а вы уже меня не слушаете, думаю нам трудно будет иметь с вами дело, поэтому сосредоточьтесь сейчас, чтобы не жалеть потом. Пока же я вынуждена поставить вам незачëт."
    elif a == 1:
        ...
        olga "{cps=45}Хоть и с долей скепсиса, но зачёт вам поставлю."
        $tpr = True
    else:
        ...
        olga "{cps=45}А я сама теперь могу с чистым сердцем поставить вам зачёт."
        $tpr = True
        

    olga "{cps=45}Ладно, на сегодня всё, надеюсь, вы внимательно меня слушали, уверена, это всё вам ещё ни раз пригодится. А пока отпускаю вас в свободное плавание, ещё увидимся."
    ...
    #поменял
    "{cps=45}На сегодня пары закончились."

    "{cps=45}Дома ждёт лучший друг студента - кровать."
